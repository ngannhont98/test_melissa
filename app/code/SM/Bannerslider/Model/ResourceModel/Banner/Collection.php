...
namespace SM\Bannerslider\Model\ResourceModel\Banner;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        $this->_init('SM\Bannerslider\Model\Banner', 'SM\Bannerslider\Model\ResourceModel\Banner');
    }
    ...
    public function __construct(
        ...
        \SM\Bannerslider\Model\Slider $slider,
        $connection = null,
    ...
    public function joinSliderTitle()
    {
        $this->getSelect()->joinLeft(
            ['sliderTable' => $this->getTable('SM_bannerslider_slider')],
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if (in_array($field, $attributes) && $storeViewId) {
            if (!in_array($field, $this->_addedTable)) {
                ...
                $this->getSelect()
                    ->joinLeft(array($field => $this->getTable('SM_bannerslider_value')), $sql, array());
    ...
    public function getBannerCollection($sliderId)
    {
        ...
        /** @var \SM\Bannerslider\Model\ResourceModel\Banner\Collection $bannerCollection */
        $bannerCollection = $this->setStoreViewId($storeViewId)
            ->addFieldToFilter('slider_id', $sliderId)
            ->addFieldToFilter('status', \SM\Bannerslider\Model\Status::STATUS_ENABLED)
            ->addFieldToFilter('start_time', ['lteq' => $dateTimeNow])
            ->addFieldToFilter('end_time', ['gteq' => $dateTimeNow])
            ->setOrder('order_banner', 'ASC');

        if ($this->_slider->getSortType() == \SM\Bannerslider\Model\Slider::SORT_TYPE_RANDOM) {
